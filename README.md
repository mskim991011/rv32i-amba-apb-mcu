...
* **UART Polling & Configurable Baud Rate:** Evaluates the `UART_STATUS` register via polling to check the `TX_READY` flag, ensuring data synchronization between the fast CPU and the slower UART transmission. The baud rate is dynamically configurable to 9600, 19200, or 115200 bps using the `UART_BAUD` register.
